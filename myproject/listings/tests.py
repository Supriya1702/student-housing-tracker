...
class ListingCRUDTestCase(TestCase):
    ...
    def test_update_listing(self):
        ...
        response = self.client.put(reverse('listing-detail', kwargs={'pk': listing.listing_id}), updated_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['title'], 'Updated Room')

    def test_delete_listing(self):
        listing = Listing.objects.create(**self.listing_data)
        response = self.client.delete(reverse('listing-detail', kwargs={'pk': listing.listing_id}))
